...
Original file is located at
    https://colab.research.google.com/github/DL-ECE/tp2-deeplearningonimages-MaximeBaude/blob/master/convolution.ipynb
...
gray_like =  build_image_like_tensor(255,255,3,(125,125,125))
...
def convolution_forward_numpy(image, kernel):
  ...
  padd = np.zeros((image.shape[0] + 2, image.shape[1] + 2))
  padd[1:-1, 1:-1] = image

  for x in range(image.shape[1]):
    for y in range(image.shape[0]):
      out[y, x]=(kernel * padd[y: y+3, x: x+3]).sum()
...
class CNNModel(nn.Module):
    def __init__(self, classes=10):
        ...
        self.layer1 = nn.Sequential(nn.Conv2d(1, 16, kernel_size=3, padding=1),nn.BatchNorm2d(16),nn.ReLU(),nn.MaxPool2d(2,stride = 2))
        self.layer2 = nn.Sequential(nn.Conv2d(16, 32, kernel_size=3, padding=1),nn.BatchNorm2d(32),nn.ReLU(),nn.MaxPool2d(2,stride = 2))
        self.layer3 = nn.Sequential(nn.Conv2d(32, 64, kernel_size=3, padding=1),nn.BatchNorm2d(64),nn.ReLU(),nn.MaxPool2d(2,stride = 2))
        self.layer4 = nn.Sequential(nn.Conv2d(64, 128, kernel_size=3, padding=1),nn.BatchNorm2d(128),nn.ReLU(),nn.MaxPool2d(2,stride = 2))
...
#Les tests ont étés réalisés en testant différentes valeurs de paramètres, 
#Contrairement aux paramètres utilisés dans le TP1, je me suis rendu compte qu'une meilleure accuracy était obtenue en baissant le learning rate à 0.001
#J'ai aussi augmenté considérablement la taille du minibatch (de 20 à 100) car les résultats étaient plus probant.
#Enfin, un nepoch de 12 ne semblait pas suffisant pour atteindre 90%, et j'ai donc passé la valeur à 18.
#A force de tests avec le momentum, je me suis rendu compte qu'utiliser une valeure autre que 0 ne me permettait pas d'atteindre une accuracy suffisante, j'ai donc mis la valeur à 0
#On utilise CNN composé de 4 layers. On utilise la fonction ReLU et l'opération maxpooling qui réduit la taille des images.
#J'avais d'abord commencé par utiliser 2 layers mais l'accuracy stagnait systématiquement à peu près à 88%, c'est en ajoutant 2 layers que j'ai réussi à dépasser les 90%
